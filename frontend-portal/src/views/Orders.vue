...
const rows = ref([]); const activeTab = ref("all"); const showReceiptUpload = ref(false); const receiptOrder = ref(null); const receiptFiles = ref([]); const receiptUploading = ref(false); const statusNames = ["待付款", "待审核", "待发货", "待收货", "已完成", "已取消", "退款中", "已退款"]; const tabs = [{ label: "待审核", status: 1 }, { label: "待发货", status: 2 }, { label: "待收货", status: 3 }, { label: "已完成", status: 4 }, { label: "退款", status: 6 }];
async function load(status = null) { try { const data = await request.get("/api/portal/order/page", { params: status == null ? {} : { status } }); rows.value = data.list; } catch (e) { showToast(e.message || "订单加载失败"); } }
function onTabChange(name) { load(name === "all" ? null : name); }
