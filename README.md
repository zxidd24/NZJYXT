...
当前自动测试共 14 项，覆盖：初始密码 BCrypt 校验、JWT 后台/门户隔离、令牌撤销、AES 加解密与脱敏、商品分类三级限制、下单令牌与支付幂等、取消回补库存、钱包余额原子变更、退款金额边界和贷款额度边界。

### P0 修复说明（2026-09-02）

- **超时取消事务**：订单超时扫描改为单事务执行，取消状态采用条件更新，库存回补与订单状态变更保持一致；不再静默吞掉异常。
- **并发支付安全**：钱包支付前使用数据库行锁读取订单，避免并发请求同时通过待付款校验并重复扣款。
...
P0 修复后的后端测试共 14 项全部通过；前端构建与接口冒烟仍按下方命令执行。

## 第9阶段：联调、测试与收尾

### 测试与联调

- 后端：`cd backend && mvn test`，当前 14 项通过。
